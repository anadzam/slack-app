...
//  slack app
//
//  Created by Ana Dzamelashvili on 11/21/22.
//
...
final class StorageManager {
    ...
    ///upload image that will bes sent to the conversation
    ...
    ///upload video that will bes sent to the conversation
    public func uploadMessageVideo(with fileUrl: URL,
                                     fileName: String,
                                     completion: @escaping uploadPictureCompletion) {
        storage.child("message_videos/\(fileName)").putFile(from: fileUrl,
                                                            metadata: nil,
